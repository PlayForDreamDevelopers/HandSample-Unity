...
namespace YVR.Interaction.Runtime
{
    public class HandRenderEffectController : MonoBehaviour
    {
        ...
        private void Start()
        {
            m_XRHandSubsystem = InputModalityManager.instance.handSubsystem;
        ...
        private void UpdatePointerPose()
        {
            ...
            if (m_XRHandSubsystem is not {running: true})
                pointer.SetActive(false);

            m_XRHand = handType == Handedness.Left ? m_XRHandSubsystem.leftHand : m_XRHandSubsystem.rightHand;
            bool showPointerWhenTracking
                = ((HandStatus) aimflag.action.ReadValue<int>() & HandStatus.InputStateValid) != 0 &&
                  m_XRHand.isTracked && !handRayInteractor.IsBlockedByInteractionWithinGroup() && pointerEnable;
